...
for (let server of global.servers) {
  if (
    ...
    continue;
  let [hostCode, regionCode] = server.code.split("-");
  let locInfo = `${global.codeTable[0][hostCode]} | ${global.codeTable[1][regionCode][0]}`;
  let tr = document.createElement("tr");
  let td = document.createElement("td");
  td.textContent = `${server.name} | ${locInfo}`;
...
var color = {};
util.pullJSON('/json/color.json').then(data => color = data);
